...
pub trait LocalStore {
    ...
    async fn existing_public_key_to_user(&self, public_key: &PublicKeyBase) -> anyhow::Result<Option<User>> {
        let user_id = self.existing_public_key_to_id(public_key).await?;
    ...
    async fn public_key_to_user(&self, public_key: &PublicKeyBase) -> anyhow::Result<User> {
        let user = self.existing_public_key_to_user(public_key).await?;
        let user = match user {
            Some(user_id) => user_id,
            None => {
                let user_id = ARID::new();
                let user = User::new(user_id.clone(), public_key.clone());
    ...
    async fn expect_public_key_to_user(&self, public_key: &PublicKeyBase) -> anyhow::Result<User> {
        let user_id = self.existing_public_key_to_user(public_key).await?;
    ...
    async fn store_share(&self, public_key: &PublicKeyBase, payload: &Bytes) -> anyhow::Result<Receipt> {
        let user = self.public_key_to_user(public_key).await?;
    ...
    async fn get_shares(&self, public_key: &PublicKeyBase, receipts: &HashSet<Receipt>) -> anyhow::Result<HashMap<Receipt, Bytes>> {
        let user = self.expect_public_key_to_user(public_key).await?;
    ...
    async fn delete_shares(&self, public_key: &PublicKeyBase, receipts: &HashSet<Receipt>) -> anyhow::Result<()> {
        let user = self.expect_public_key_to_user(public_key).await?;
    ...
    async fn update_public_key(&self, old_public_key: &PublicKeyBase, new_public_key: &PublicKeyBase) -> anyhow::Result<()> {
        if self.existing_public_key_to_id(new_public_key).await?.is_some() {
            bail!("public key already in use");
        }
        self.set_user_public_key(old_public_key, new_public_key).await?;
        Ok(())
    }
    ...
    async fn delete_account(&self, public_key: &PublicKeyBase) -> anyhow::Result<()> {
        let user = self.expect_public_key_to_user(public_key).await?;
        self.delete_shares(public_key, &HashSet::new()).await?;
    ...
    /// the reset.
    async fn start_fallback_transfer(&self, fallback: &str, new_public_key: &PublicKeyBase) -> anyhow::Result<FallbackContinuation> {
        ...
        let existing_user = self.existing_public_key_to_id(new_public_key).await?;
    ...
    async fn finish_fallback_transfer(&self, continuation: &FallbackContinuation) -> anyhow::Result<()> {
        ...
        self.set_user_public_key(continuation.old_public_key(), continuation.new_public_key()).await?;
        Ok(())
    }
}
...
pub struct FallbackContinuation {
    pub old_public_key: PublicKeyBase,
    pub new_public_key: PublicKeyBase,
    pub expiry: dcbor::Date,
}

impl FallbackContinuation {
    pub fn new(old_public_key: PublicKeyBase, new_public_key: PublicKeyBase, expiry: dcbor::Date) -> Self {
        Self {
            old_public_key,
            new_public_key,
            expiry,
    ...
    pub fn old_public_key(&self) -> &PublicKeyBase {
        &self.old_public_key
    }

    pub fn new_public_key(&self) -> &PublicKeyBase {
        &self.new_public_key
    }
